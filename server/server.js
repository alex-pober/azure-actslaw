...
if (
  ...
  console.warn(
    "Azure AI Search is not fully configured. RAG functionality will be limited."
  );
}

if (!AZURE_TENANT_ID || !BACKEND_CLIENT_ID || !FRONTEND_CLIENT_ID) {
  console.error(
    "Please set the required Azure AD environment variables for JWT validation"
  );
  process.exit(1);
}
...
if (
  ...
  try {
    ...
    console.log(
      `Search client initialized for index: ${AZURE_AI_SEARCH_INDEX}`
    );
    ...
    try {
      ...
      index.fields.forEach((field) => {
        console.log(
          `- ${field.name} (${field.type}) - searchable: ${field.searchable}, retrievable: ${field.retrievable}`
        );
      });
    } catch (schemaError) {
  ...
  try {
    ...
    if (sources.length > 0) {
      console.log(
        ...
        Object.keys(sources[0].document)
      );
    }
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const { response: openaiResponse, sources: finalSources } =
        await queryOpenAIForResponse(
